...
**変更内容:**
- Bluesky画像は `fullsize` ではなく `thumb` を優先して取得
- 単一画像でもそのまま送らず、320px上限・JPEG品質50で縮小
- 画像グリッドの最大サイズを 1200px → 800px に削減

**効果:** 画像付き投稿でのBase64変換量とピクセル処理量を大幅に削減

#### 9. Base64変換のネイティブ化

**変更内容:**
- `nodejs_compat` を有効化
- 手動ループによる `btoa` 変換をやめ、`Buffer.from(...).toString('base64')` を使用

**効果:** 画像エンコード時のJavaScriptループ負荷を削減

#### 10. 処理済み投稿・通知のKV更新を1回に集約

**変更内容:**
- 1件処理ごとの `get → JSON.parse → push → JSON.stringify → put` を廃止
- 実行中はメモリ上で追跡し、最後に1回だけKVへ保存
- 古いエントリの整理も読み込み時にまとめて実施

**効果:** 投稿数・通知数が増えたときのKV周りのCPU負荷を抑制

#### 11. 投稿検索範囲の縮小

**変更内容:**
- 毎回24時間分を検索するのではなく、直近の評価時刻から1時間だけ余裕を持たせて再検索

**効果:** 同じ投稿群を何度もなめる処理を減らし、定期実行あたりの処理件数を削減
...
- 2026-02-17: 追加最適化実装（Issue #25対応）
  - console.logの削減（13箇所 → 0箇所、ログ処理時間100%削減）
  - 画像サイズのさらなる削減（512x512 → 400x400、約35%削減）
  - グリッド画像サイズの削減（1600x1600 → 1200x1200、約44%削減）
  - JPEG品質の低下（70 → 60、約15%処理時間削減）
  - API制限のさらなる削減（30 → 20、約33%削減）
  - エラーログの簡潔化（スタックトレース削除）
- 2026-08-30: CPU timeout 再発への追加最適化
  - サムネイル画像の利用と単一画像の縮小
  - グリッド画像サイズを 800x800、JPEG品質を 50 に削減
  - Base64変換を `Buffer` ベースへ置き換え
  - 処理済み投稿・通知のKV更新を1実行1回に集約
  - 投稿検索範囲を最終評価時刻ベースに縮小
